...
| 全サービス停止（コンテナ削除） | `docker compose down` |
| 全サービス停止（コンテナ保持） | `docker compose stop` |
| 全サービス再起動 | `docker compose restart` |
| 起動状態確認 | `docker compose ps` |
| ログ確認（全サービス） | `docker compose logs -f` |

### 個別サービスの起動・停止

```bash
# サービス停止（シナリオ障害注入）
docker compose stop payment          # シナリオA：決済サービス停止
docker compose stop product-reviews  # シナリオB：LLMチャット停止

# サービス起動（復旧）
docker compose start payment
docker compose start product-reviews

# サービス再起動（環境変数変更後など）
docker compose restart product-reviews
```

### シナリオB：LLMチャット障害（環境変数変更）

```bash
# 障害注入：無効なAPIキーで product-reviews を再起動
OPENAI_API_KEY=invalid_key_for_demo docker compose up -d product-reviews

# 復旧：.env.override の正しいキーで再起動（ファイルから読み込み）
docker compose up -d product-reviews
```

### シナリオC：メモリリーク（feature flag）

`src/flagd/demo.flagd.json` の `emailMemoryLeak` を編集して flagd に反映：

```json
"emailMemoryLeak": {
  "defaultVariant": "100x"   // 変更（off → 100x）
}
```

復旧時は `"defaultVariant": "off"` に戻す。flagd はファイル変更を自動検知するため再起動不要。

### デモリセット（全シナリオ共通）

```bash
# 停止したサービスを全て再起動
docker compose start payment product-reviews

# feature flag をすべて off に戻す（flagd の JSON を編集後、自動反映）
# src/flagd/demo.flagd.json の defaultVariant を "off" に変更
